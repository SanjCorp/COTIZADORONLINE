...
public sealed class QuotesController(AppDbContext db,BusinessSettingsService settings):ControllerBase
{
    ...
    public async Task<IActionResult>Create(QuoteRequest request,CancellationToken ct)
    {
        ...
        db.Quotes.Add(quote);await db.SaveChangesAsync(ct);await transaction.CommitAsync(ct);return CreatedAtAction(nameof(Get),new{id=quote.Id},new{quote.Id,quote.OrderCode,quote.CreatedAtUtc,quote.Customer,quote.ProjectName,quote.PrinterName,quote.TotalWeight,CostTotal=quote.Subtotal,quote.RecommendedPrice,SoldAtUtc=(DateTime?)null});
    }
    ...
    {
        var quote=await db.Quotes.Include(x=>x.Sale).FirstOrDefaultAsync(x=>x.Id==id,ct);if(quote is null)return NotFound();if(quote.Sale is not null)return Conflict(new{message="Esta cotización ya fue confirmada como venta."});
        quote.Sale=new Sale{SoldAtUtc=DateTime.UtcNow,SaleAmount=quote.RecommendedPrice};await db.SaveChangesAsync(ct);return Ok(new{quote.Sale.Id,quote.Sale.QuoteId,quote.Sale.SoldAtUtc,quote.Sale.SaleAmount});
    }

    [Authorize(Roles=AppRoles.Administrator),HttpDelete("{id:long}")]
    public async Task<IActionResult>Delete(long id,CancellationToken ct){var quote=await db.Quotes.FindAsync([id],ct);if(quote is null)return NotFound();db.Quotes.Remove(quote);await db.SaveChangesAsync(ct);return NoContent();}
