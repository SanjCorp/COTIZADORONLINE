...
function Login({ onSuccess }: { onSuccess: (profile: Profile) => void }) {
  const [username, setUsername] = useState(''); const [password, setPassword] = useState(''); const [code, setCode] = useState(''); const [workspace, setWorkspace] = useState('technology'); const [requiresCode, setRequiresCode] = useState(false); const [error, setError] = useState(''); const [busy, setBusy] = useState(false)
  ...
  return <main className="login-shell"><section className="brand-panel"><div className="brand-mark"><Boxes size={34} /></div><p className="eyebrow">SANJ CORP TECHNOLOGY</p><h1>Control preciso para cada impresión.</h1><p className="lead">Cotizaciones, inventario y ventas en un espacio privado diseñado para tu equipo.</p><div className="security-note"><ShieldCheck size={20} /><span>Acceso protegido, permisos por rol y datos cifrados.</span></div></section><section className="login-panel"><form className="login-card" onSubmit={submit}><div className="lock"><LockKeyhole size={24} /></div><p className="eyebrow">ACCESO AL SISTEMA</p><h2>Iniciar sesión</h2><p className="muted">Elige el espacio de trabajo de tu cuenta.</p><div className="workspace-choice"><button type="button" className={workspace === 'technology' ? 'selected' : ''} onClick={() => setWorkspace('technology')}>SanjCorp Technology</button><button type="button" className={workspace === 'makers' ? 'selected' : ''} onClick={() => setWorkspace('makers')}>Makers</button></div><label>Usuario<input type="text" autoComplete="username" value={username} onChange={event => setUsername(event.target.value)} required autoFocus /></label><label>Contraseña<input type="password" autoComplete="current-password" value={password} onChange={event => setPassword(event.target.value)} required /></label>{requiresCode && <label>Código 2FA o de recuperación<input inputMode="numeric" autoComplete="one-time-code" value={code} onChange={event => setCode(event.target.value)} required autoFocus /></label>}{error && <p className="error" role="alert">{error}</p>}<button disabled={busy}>{busy ? 'Verificando…' : 'Entrar de forma segura'}</button></form></section></main>
}
