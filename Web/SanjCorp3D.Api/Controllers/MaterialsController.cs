...
public sealed class MaterialsController(AppDbContext db):ControllerBase
{
    ...
    [Authorize(Roles=$"{AppRoles.Administrator},{AppRoles.Production}"),HttpPost]public async Task<IActionResult>Create(ExtraMaterial input,CancellationToken ct){input.Id=0;input.Active=true;Validate(input);db.Materials.Add(input);return await Save(input,ct);}
    ...
    [Authorize(Roles=$"{AppRoles.Administrator},{AppRoles.Production}"),HttpDelete("{id:long}")]public async Task<IActionResult>Archive(long id,CancellationToken ct){var item=await db.Materials.FindAsync([id],ct);if(item is null)return NotFound();item.Active=false;await db.SaveChangesAsync(ct);return NoContent();}
